// TestRotor.ino
// to be loaded on APM 
// drives each ESCs for 1s 1 at a time to check if motors are operational
...
void setupESC(){ // Setup ESCs for top motors.
  ...
  Serial.println(F("Turn on the mofo. Enter speed to continue."));
  while (Serial.available() && Serial.read()); // empty buffer
  while (!Serial.available()); // wait for data
  firstESCSpeed = Serial.parseInt();
  while (Serial.available() && Serial.read()); // empty buffer again  
  Serial.println("Flag");
}
