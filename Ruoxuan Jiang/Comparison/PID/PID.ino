...
void setup()
{
  Serial.begin(115200);
  setupMPU();
  SetTunings(55, 0.01, 8000);// original 41, 0.0005, 8000: Stable @1000 Minspeed
  setupESC();
  timeBegin = millis();
  //timeStamp = 0;
}
...
void turnOff(){ // Turns off the motors after 'endTime' milliseconds. Can then restart the test with new gains or the same.
  ...
  Serial.print("Begin testing PID gains of: ");
  Serial.print(newP);
  Serial.print(", ");
  Serial.print(newI);
  Serial.print(", ");
...
{ // Setup MPU6050 using the DMP.
    ...
    mpu.setYGyroOffset(20); // 20
    mpu.setZGyroOffset(-69); // -69
    mpu.setXAccelOffset(-5679); // -5679
    mpu.setYAccelOffset(2988); // 2988
    mpu.setZAccelOffset(1532); //1532 1688 factory default for my test chip
...
{ // Setup ESCs for top motors.
  ...
  thirdESC.attach(9); // orig 12
  fourthESC.attach(6); // orig 13
  while (!Serial);
...
{ // Control feedback goes in here.
  ...
  //  if (now > 1000 && timeStamp < 400){
  //    timeData[timeStamp] = (unsigned short) timeChange;
  //    timeStamp ++;
  //  }
   ...
   thirdESCSpeed = -(kp * pitchError + ki * pitchErrSum - kd * dPitch/timeChange);
   ...
   fourthESCSpeed = (kp * pitchError + ki * pitchErrSum - kd * dPitch/timeChange);
